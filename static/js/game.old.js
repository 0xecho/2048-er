...
function next_move(shouldUpdateDom = true) {
  move(MOVES[move_index++], shouldUpdateDom);
}

function move(whichKey, shouldUpdateDom = true) {
  const prevGame = [...game];
  ...
  addRandomNumber();
  if (shouldUpdateDom) updateDOM(prevGame, game);
}
